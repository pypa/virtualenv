    ...
    def run(self):
        if self.options.no_venv is False and self.interpreter.has_venv:
            ...
            if self.env_dir.exists() and self.options.clear:
                shutil.rmtree(str(self.env_dir), ignore_errors=True)
            ...
            try:
                ...
                if not self.options.without_pip:
                    logging.debug(
                        "seed base packages with %s form %s",
                        self._seed_packages.__name__,
                        inspect.getmodule(self._seed_packages).__name__,
                    )
