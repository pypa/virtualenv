...
from virtualenv.seed.embed.wheels.acquire import Version, WheelDownloadFail, get_wheel
...
class FromAppData(BaseEmbed):
    ...
    def __init__(self, options):
        ...
        self.base_cache = self.app_data / "seed-app-data" / "v1.0.2"
    ...
    def _get_seed_wheels(self, creator, base_cache):
        with base_cache.lock_for_key("wheels"):
            ...
            def _get(pkg, version):
                ...
                loader = partial(get_wheel, pkg, version, for_py_version, self.extra_search_dir)
                ...
                if failure:
                    if isinstance(failure, WheelDownloadFail):
                        msg = "failed to download {}".format(pkg)
