...
def pip_install(wheels, creator, cache):
    ...
    for name, wheel in wheels.items():
        ...
        for extra in _generate_extra_files(bin_dir, env_exe, site_package, dist_info, creator):
            logging.debug("write %s", extra)
...
def _get_extracted(cache, wheel):
    unwrapped_wheels = cache / "extract"
    unwrapped_wheels.mkdir(parents=True, exist_ok=True)
    extracted = unwrapped_wheels / wheel.stem
    if not extracted.exists():
        logging.debug("extract wheel to %s from %s", extracted, wheel)
        extracted.mkdir(parents=True, exist_ok=True)
        with zipfile.ZipFile(str(wheel)) as zip_ref:
            zip_ref.extractall(str(extracted))
    else:
        logging.debug("use extracted wheel from %s", extracted)
    return extracted


def _link_content(folder_link, site_package, extracted):
    logging.debug("link %s to %s", extracted, site_package)
    added = []
    dist_info = None
...
def _generate_extra_files(bin_dir, env_exe, site_package, dist_info, creator):
    yield write_installer(dist_info)
    yield write_marker(dist_info, site_package)
...
def write_marker(dist_info, site_package):
