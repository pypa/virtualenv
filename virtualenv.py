...
def fix_libdir_bitness(lib_dir, symlink=True):
    """
    Some platforms (like Gentoo on x64 or on MIPS ABI n32/n64) put things in
    lib{64,32}/pythonX.Y instead of lib/pythonX.Y.  If this is such a
...
def _do_fix_libdir_bitness(lib_dir, symlink, bitness_str):
    target_dirname = 'lib%s' % (bitness_str, )
